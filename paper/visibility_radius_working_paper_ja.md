...
- Figure 2: Phantom candidates と actionable reserve の比較
- Figure 3: Actionable reserve protected-slot threshold
- Figure 4: 長期文化適応シナリオ
- Figure 5: Region 境界と文化別推移
- Figure 6: 行動介入シナリオ
...
Figure 1 は、SNS 2000s 型の可視半径拡大シナリオにおける簡易表示である。以降の図では、この崩壊的な baseline に対して、どの介入がどの程度人口推移を回復させるかを見る。
...
![Figure 2. Phantom candidates と actionable reserve の比較。](figures/actionable_attention_comparison.png)

Figure 2 は、phantom candidates が top-k 注意枠を消費する場合と、actionable reserve によって実行可能候補を保護する場合を比較している。人口維持の差は、出生率そのものよりも、実行可能候補が注意枠に残るかどうかに強く結びついている。
...
Figure 3 は、reserve fraction を top-k 内の保護スロット数として見た場合の閾値的な推移を示す。`top_k=16` では、0 スロットから 4-6 スロット付近までが大きな移行領域になる。
...
![Figure 4. 長期文化適応シナリオ。](figures/civilizational_adaptation_longrun.png)

Figure 4 は、補正なし、弱い補正、balanced 補正、強い補正、local-only reference の長期推移を示す。補正が閾値を超えると、生存か絶滅かよりも、回復深度や人口比率の違いが中心になる。
...
Figure 5a と Figure 5b は、Region 境界と文化シェアの関係を示す。境界は総人口だけでなく、弱・中・強の reserve culture がどの程度残るかにも影響する。
...
![Figure 6. 行動介入シナリオ。](figures/behavioral_action_interventions.png)

Figure 6 は、個人行動に近い形へ翻訳した介入の比較である。最も効いたのは「仮想比較前に現実接触枠を確保する」系統であり、phantom candidates の反復を減らすだけでは十分でなかった。
...
本稿の後に、七つの追試 A-G を追加した。結果は `paper/appendix_followups/` にまとめている。

追試の主な更新点は次である。

- Radius 拡大単独ではなく、actionability gap と phantom comparison が組み合わさると崩壊が強くなる。
- SNS 的効果は、空間距離よりも perceived candidate-pool multiplier として表す方が強く出る。
- top-k attention は脆いが、percentile selection だけでは十分な救済にならない。
- action radius を広げるだけでは、極端な phantom competition を止められない。
- protected actionable slots の閾値構造は `top_k=8,16,32,64` でも概ね残った。
- 現行モデルでは、強すぎる reserve culture の過剰制約ペナルティはまだ出ていない。
- 地域・制度が人口低下を観察して reserve culture を上げる場合、十分早い学習なら崩壊を大きく緩和できる。

この補遺は、本稿の主張を「Radius 拡大が悪い」から、次へさらに絞り込む。

```text
非実行可能比較が bounded attention を占有することが問題であり、
十分な actionable slots または制度的学習が必要である
```
...
この補遺には、利用 script、seed、主要 parameter、raw/summary output の場所、残る再現性ギャップを記録している。
